...
public class SuggestionsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetStoreSuggestions([FromQuery] string query)
    {
        ...
        // Нормализация запроса для кеширования
        ...
        // Назначение частей запроса соответствующим полям
        // Предполагаем порядок: Building, Floor, Line, StoreNumber
        ...
        if (queryParts.Length > 0)
            building = queryParts[0];
        if (queryParts.Length > 1)
            floor = queryParts[1];
        if (queryParts.Length > 2)
            line = queryParts[2];
        if (queryParts.Length > 3)
            storeNumber = queryParts[3];

        // Начальный запрос
        var suggestionsQuery = _context.Stores.AsQueryable();

        // Применение фильтров с использованием ILIKE
        if (!string.IsNullOrEmpty(building))
        {
            string buildingFilter = $"{building.Trim()}%";
            suggestionsQuery = suggestionsQuery.Where(s => EF.Functions.ILike(s.Building, buildingFilter));
        }

        if (!string.IsNullOrEmpty(floor))
        {
            string floorFilter = $"{floor.Trim()}%";
            suggestionsQuery = suggestionsQuery.Where(s => EF.Functions.ILike(s.Floor, floorFilter));
        }

        if (!string.IsNullOrEmpty(line))
        {
            string lineFilter = $"{line.Trim()}%";
            suggestionsQuery = suggestionsQuery.Where(s => EF.Functions.ILike(s.Line, lineFilter));
        }

        if (!string.IsNullOrEmpty(storeNumber))
        {
            string storeNumberFilter = $"{storeNumber.Trim()}%";
            suggestionsQuery = suggestionsQuery.Where(s => EF.Functions.ILike(s.StoreNumber, storeNumberFilter));
        }
